    ...
    socketIOService.on('user_updated', (data: any) => {
      setUsers(prev => prev.map(u =>
        u.id === data.id?.toString() || u.email === data.email
          ? { ...u, ...data, id: data.id?.toString() || u.id }
          : u
      ));
    });
    ...
    try {
      ...
      if (res.ok) {
        if (editingId) {
          ...
          setUsers([...users, { id: newUser.id.toString(), email: newUser.email, name: newUser.name, role: newUser.role, isActive: newUser.is_active, createdAt: newUser.created_at }]);
        }
        setShowModal(false);
